var Crypto = {
    ...
    bytesToBase64: function (bytes) {
        ...
        for (var i = 0; i < bytes.length; i++) {
            switch (i % 3) {
                case 0:
                    base64.push(Crypto.base64map.charAt(bytes[i] >>> 2));
                    overflow = (bytes[i] & 0x3) << 4;
                    break;
                case 1:
                    base64.push(Crypto.base64map.charAt(overflow | (bytes[i] >>> 4)));
                    overflow = (bytes[i] & 0xF) << 2;
                    break;
                case 2:
                    base64.push(Crypto.base64map.charAt(overflow | (bytes[i] >>> 6)));
                    base64.push(Crypto.base64map.charAt(bytes[i] & 0x3F));
                    overflow = -1;
            }
        }
        ...
        if (overflow != undefined && overflow != -1)
            base64.push(Crypto.base64map.charAt(overflow));
    ...
    _sha1: function (message) {
        ...
        var m = Crypto.stringToWords(message),
            l = message.length * 8,
            w = [],
            H0 = 1732584193,
            H1 = -271733879,
    ...
    SHA1: function (message, options) {
        var digestbytes = Crypto.wordsToBytes(Crypto._sha1(message));
    ...
    HMAC: function (hasher, message, key, options) {
        ...
        return options && options.asBytes ? hmacbytes :
            options && options.asString ? bytesToString(hmacbytes) :
